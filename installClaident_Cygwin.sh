...
wget -c ftp://ftp.ncbi.nlm.nih.gov/blast/executables/blast+/2.8.1/ncbi-blast-2.8.1+-x64-win64.tar.gz || exit $?
tar -xzf ncbi-blast-2.8.1+-x64-win64.tar.gz || exit $?
cd ncbi-blast-2.8.1+/bin || exit $?
mkdir -p $PREFIX/share/claident/bin || exit $?
mv * $PREFIX/share/claident/bin/ || exit $?
cd ../.. || exit $?
rm -rf ncbi-blast-2.8.1+ || exit $?
rm -f ncbi-blast-2.8.1+-x64-win64.tar.gz || exit $?
touch .blast || exit $?
fi
