...
wget -c https://github.com/astanabe/Claident/archive/v0.2.2017.05.06.tar.gz -O Claident-0.2.2017.05.06.tar.gz || exit $?
tar -xzf Claident-0.2.2017.05.06.tar.gz || exit $?
cd Claident-0.2.2017.05.06 || exit $?
sh install_on_OSX.sh || exit $?
cd .. || exit $?
...
wget -c ftp://ftp.ncbi.nlm.nih.gov/blast/executables/blast+/2.6.0/ncbi-blast-2.6.0+-universal-macosx.tar.gz || exit $?
gnutar -xzf ncbi-blast-2.6.0+-universal-macosx.tar.gz || exit $?
...
rm -f ncbi-blast-2.6.0+-universal-macosx.tar.gz || exit $?
touch .blast || exit $?
fi
