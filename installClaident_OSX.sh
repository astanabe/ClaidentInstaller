...
gnutar -xzf vsearch-1.9.10.tar.gz || exit $?
...
wget -c ftp://ftp.ncbi.nih.gov/blast/executables/blast+/2.2.31/ncbi-blast-2.2.31+-universal-macosx.tar.gz || exit $?
gnutar -xzf ncbi-blast-2.2.31+-universal-macosx.tar.gz || exit $?
...
rm -f ncbi-blast-2.2.31+-universal-macosx.tar.gz || exit $?
touch .blast || exit $?
fi
