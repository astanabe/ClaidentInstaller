...
gnutar -xzf vsearch-2.13.3.tar.gz || exit $?
...
gnutar -xzf vsearch5d-2.13.3.tar.gz || exit $?
...
wget -c ftp://ftp.ncbi.nlm.nih.gov/blast/executables/blast+/2.9.0/ncbi-blast-2.9.0+-x64-macosx.tar.gz || exit $?
gnutar -xzf ncbi-blast-2.9.0+-x64-macosx.tar.gz || exit $?
...
rm -f ncbi-blast-2.9.0+-x64-macosx.tar.gz || exit $?
touch .blast || exit $?
fi
