...
wget -nv -c https://mafft.cbrc.jp/alignment/software/mafft-7.487-mac.zip || exit $?
unzip -qq mafft-7.487-mac.zip || exit $?
mkdir -p $PREFIX/share/claident/bin 2> /dev/null || sudo mkdir -p $PREFIX/share/claident/bin || exit $?
...
rm -rf mafft-7.487-mac.zip mafft-mac || exit $?
...
gnutar -xzf Metaxa2_2.2.3.tar.gz || exit $?
mkdir -p $PREFIX/share/claident/bin 2> /dev/null || sudo mkdir -p $PREFIX/share/claident/bin || exit $?
