...
gnutar -xzf hmmer-3.1b2-macosx-intel.tar.gz || exit $?
...
wget -nv -c https://mafft.cbrc.jp/alignment/software/mafft-7.475-mac.zip || exit $?
unzip -qq mafft-7.475-mac.zip || exit $?
mkdir -p $PREFIX/share/claident/bin 2> /dev/null || sudo mkdir -p $PREFIX/share/claident/bin || exit $?
...
rm -rf mafft-7.475-mac.zip mafft-mac || exit $?
echo 'MAFFT was installed correctly!'
touch .mafft || exit $?
fi
# download and install Metaxa
if ! test -e .metaxa; then
wget -nv -c https://microbiology.se/sw/Metaxa2_2.2.2.tar.gz || exit $?
gnutar -xzf Metaxa2_2.2.2.tar.gz || exit $?
mkdir -p $PREFIX/share/claident/bin 2> /dev/null || sudo mkdir -p $PREFIX/share/claident/bin || exit $?
rm -rf $PREFIX/share/claident/bin/metaxa2_db 2> /dev/null || sudo rm -rf $PREFIX/share/claident/bin/metaxa2_db || exit $?
mv Metaxa2_2.2.2/metaxa2 Metaxa2_2.2.2/metaxa2_* $PREFIX/share/claident/bin/ 2> /dev/null || sudo mv Metaxa2_2.2.2/metaxa2 Metaxa2_2.2.2/metaxa2_* $PREFIX/share/claident/bin/ || exit $?
rm -rf Metaxa2_2.2.2.tar.gz Metaxa2_2.2.2 || exit $?
...
gnutar -xzf ITSx_1.1.3.tar.gz || exit $?
mkdir -p $PREFIX/share/claident/bin 2> /dev/null || sudo mkdir -p $PREFIX/share/claident/bin || exit $?
