...
gtar -xzf Claident-0.9.2025.09.29.tar.gz || exit $?
...
gtar -xzf vsearch-2.30.1.tar.gz || exit $?
...
gtar -xzf vsearch5d-2.30.1.tar.gz || exit $?
...
intlprefix=`find $HOMEBREW_PREFIX/Cellar -name libintl.dylib | sort | tail -n 1 | perl -npe 's/\/lib\/libintl\.dylib$//'`
