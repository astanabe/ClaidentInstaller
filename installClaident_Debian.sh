sudo -E apt-get -y install gcc g++ gfortran make autoconf automake zlib1g-dev libbz2-dev liblzma-dev libreadline-dev libpcre2-dev libjpeg-dev libpng-dev libcairo2-dev libpango1.0-dev libtiff-dev tcl-dev tk-dev tar gzip xz-utils unzip coreutils grep perl libdbi-perl libdbd-sqlite3-perl libwww-perl libfile-copy-recursive-perl libstatistics-distributions-perl libio-compress-lzma-perl libmath-basecnv-perl libopenblas-serial-dev libopenblas64-serial-dev libcurl4-openssl-dev libxml2-dev || exit $?
if test -z $PREFIX; then
export PREFIX=/usr/local || exit $?
fi
# download, and install Claident
if ! test -e .claident; then
wget -nv -c https://github.com/astanabe/Claident/archive/v0.9.2021.10.19.tar.gz -O Claident-0.9.2021.10.19.tar.gz || exit $?
tar -xzf Claident-0.9.2021.10.19.tar.gz || exit $?
cd Claident-0.9.2021.10.19 || exit $?
...
cd .. || exit $?
rm -rf Claident-0.9.2021.10.19 || exit $?
rm -f Claident-0.9.2021.10.19.tar.gz || exit $?
touch .claident || exit $?
fi
...
wget -nv -c https://github.com/torognes/vsearch/archive/v2.18.0.tar.gz -O vsearch-2.18.0.tar.gz || exit $?
tar -xzf vsearch-2.18.0.tar.gz || exit $?
cd vsearch-2.18.0 || exit $?
sh ./autogen.sh || exit $?
...
cd .. || exit $?
rm -rf vsearch-2.18.0 || exit $?
rm -f vsearch-2.18.0.tar.gz || exit $?
touch .vsearch || exit $?
fi
# download , compile, and install VSEARCH5D
if ! test -e .vsearch5d; then
wget -nv -c https://github.com/astanabe/vsearch5d/archive/v2.18.0.tar.gz -O vsearch5d-2.18.0.tar.gz || exit $?
tar -xzf vsearch5d-2.18.0.tar.gz || exit $?
cd vsearch5d-2.18.0 || exit $?
sh ./autogen.sh || exit $?
...
make install-exec 2> /dev/null || sudo make install-exec || exit $?
cd .. || exit $?
rm -rf vsearch5d-2.18.0 || exit $?
rm -f vsearch5d-2.18.0.tar.gz || exit $?
touch .vsearch5d || exit $?
fi
# download, and install BLAST+
if ! test -e .blast; then
wget -nv -c https://ftp.ncbi.nlm.nih.gov/blast/executables/blast+/2.12.0/ncbi-blast-2.12.0+-x64-linux.tar.gz || exit $?
tar -xzf ncbi-blast-2.12.0+-x64-linux.tar.gz || exit $?
...
rm -f ncbi-blast-2.12.0+-x64-linux.tar.gz || exit $?
touch .blast || exit $?
fi
# download, compile, and install R and DADA2
if ! test -e .dada2; then
wget -nv -c https://cran.r-project.org/src/base/R-4/R-4.1.1.tar.gz || exit $?
tar -xzf R-4.1.1.tar.gz || exit $?
cd R-4.1.1 || exit $?
...
$PREFIX/share/claident/bin/R --vanilla -e 'options(download.file.method="wget");library(parallel);install.packages("RcppParallel",repos="https://cloud.r-project.org/",dependencies=T,clean=T,Ncpus=detectCores())' || exit $?
...
sudo -E $PREFIX/share/claident/bin/R --vanilla -e 'options(download.file.method="wget");library(parallel);install.packages("RcppParallel",repos="https://cloud.r-project.org/",dependencies=T,clean=T,Ncpus=detectCores())' || exit $?
