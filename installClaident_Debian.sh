...
./configure --prefix=$PREFIX/share/claident --enable-java=no --with-recommended-packages=no --with-pic --with-x=no --enable-R-shlib=no --with-blas="-L$openblas -lopenblas" --with-lapack r_cv_have_curl728=yes || exit $?
make -j$NCPU || exit $?
