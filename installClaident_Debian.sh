...
wget -c https://github.com/astanabe/Claident/archive/v0.9.2020.11.11.tar.gz -O Claident-0.9.2020.11.11.tar.gz || exit $?
tar -xzf Claident-0.9.2020.11.11.tar.gz || exit $?
cd Claident-0.9.2020.11.11 || exit $?
...
cd .. || exit $?
rm -rf Claident-0.9.2020.11.11 || exit $?
rm -f Claident-0.9.2020.11.11.tar.gz || exit $?
touch .claident || exit $?
fi
...
tar -xzf vsearch-2.15.0.tar.gz || exit $?
...
tar -xzf vsearch5d-2.15.0.tar.gz || exit $?
...
wget -c ftp://ftp.ncbi.nlm.nih.gov/blast/executables/blast+/2.10.1/ncbi-blast-2.10.1+-x64-linux.tar.gz || exit $?
tar -xzf ncbi-blast-2.10.1+-x64-linux.tar.gz || exit $?
...
rm -f ncbi-blast-2.10.1+-x64-linux.tar.gz || exit $?
touch .blast || exit $?
fi
...
make -j8 || exit $?
make install-strip || sudo make install-strip || exit $?
cd .. || exit $?
rm -rf R-4.0.3 || exit $?
