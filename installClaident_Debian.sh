sudo -E apt -y install build-essential curl libidn12 libnet-perl wget libsqlite3-dev cmake cpio libgomp1 libssl-dev liblmdb-dev liblmdb++-dev gcc g++ gfortran make autoconf automake zlib1g-dev libbz2-dev liblzma-dev libreadline-dev libpcre2-dev libjpeg-dev libpng-dev libcairo2-dev libpango1.0-dev libtiff-dev tcl-dev tk-dev tar gzip bzip2 xz-utils pigz pbzip2 lbzip2 unzip coreutils grep perl libdbi-perl libdbd-sqlite3-perl libwww-perl libfile-copy-recursive-perl libmath-basecnv-perl libopenblas-serial-dev libopenblas64-serial-dev libcurl4-openssl-dev libxml2-dev imagemagick git fonts-noto aria2 || exit $?
...
./cpanm -nfL $PREFIX/share/claident Math::CDF || exit $?
else
sudo -E mkdir -p $PREFIX/share/claident || exit $?
sudo -E ./cpanm -nfL $PREFIX/share/claident Math::CDF || exit $?
fi
...
tar -xzf Claident-0.9.2025.09.29.tar.gz || exit $?
...
wget -c https://github.com/torognes/vsearch/archive/v2.30.1.tar.gz -O vsearch-2.30.1.tar.gz || exit $?
tar -xzf vsearch-2.30.1.tar.gz || exit $?
cd vsearch-2.30.1 || exit $?
sh ./autogen.sh || exit $?
...
cd .. || exit $?
rm -rf vsearch-2.30.1 || exit $?
rm -f vsearch-2.30.1.tar.gz || exit $?
touch .vsearch || exit $?
fi
# download, compile, and install VSEARCH5D
if ! test -e .vsearch5d; then
wget -c https://github.com/astanabe/vsearch5d/archive/v2.30.1.tar.gz -O vsearch5d-2.30.1.tar.gz || exit $?
tar -xzf vsearch5d-2.30.1.tar.gz || exit $?
cd vsearch5d-2.30.1 || exit $?
sh ./autogen.sh || exit $?
...
make install-exec 2> /dev/null || sudo make install-exec || exit $?
cd .. || exit $?
rm -rf vsearch5d-2.30.1 || exit $?
rm -f vsearch5d-2.30.1.tar.gz || exit $?
touch .vsearch5d || exit $?
fi
# download, and install BLAST+
if ! test -e .blast; then
if test ${arch} = 'x86_64'; then
wget -c https://ftp.ncbi.nih.gov/blast/executables/blast+/2.17.0/ncbi-blast-2.17.0+-x64-linux.tar.gz || exit $?
tar -xzf ncbi-blast-2.17.0+-x64-linux.tar.gz || exit $?
...
rm -f ncbi-blast-2.17.0+-x64-linux.tar.gz || exit $?
elif test ${arch} = 'aarch64'; then
wget -c https://ftp.ncbi.nih.gov/blast/executables/blast+/2.17.0/ncbi-blast-2.17.0+-aarch64-linux.tar.gz || exit $?
tar -xzf ncbi-blast-2.17.0+-aarch64-linux.tar.gz || exit $?
mv -f ncbi-blast-2.17.0+/bin/* $PREFIX/share/claident/bin/ 2> /dev/null || sudo mv -f ncbi-blast-2.17.0+/bin/* $PREFIX/share/claident/bin/ || exit $?
rm -rf ncbi-blast-2.17.0+ || exit $?
rm -f ncbi-blast-2.17.0+-aarch64-linux.tar.gz || exit $?
else
wget -c https://ftp.ncbi.nih.gov/blast/executables/blast+/2.17.0/ncbi-blast-2.17.0+-src.tar.gz || exit $?
tar -xzf ncbi-blast-2.17.0+-src.tar.gz || exit $?
...
CFLAGS="-Wno-error=incompatible-pointer-types -Wno-error=implicit-function-declaration -Wno-error=int-conversion" ./configure --prefix=$PREFIX/share/claident --enable-java=no --with-recommended-packages=yes --with-pic --with-x=no --enable-R-shlib=yes --enable-nls=no --with-ICU=no --with-blas="-L$openblas -lopenblas" --with-lapack || exit $?
make -j$NCPU || exit $?
