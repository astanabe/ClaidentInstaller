...
wget -nv -c https://mafft.cbrc.jp/alignment/software/mafft-7.475-linux.tgz || exit $?
tar -xzf mafft-7.475-linux.tgz || exit $?
mkdir -p $PREFIX/share/claident/bin 2> /dev/null || sudo mkdir -p $PREFIX/share/claident/bin || exit $?
...
rm -rf mafft-7.475-linux.tgz mafft-linux64 mafft-linux32 || exit $?
...
tar -xzf Metaxa2_2.2.2.tar.gz || exit $?
mkdir -p $PREFIX/share/claident/bin 2> /dev/null || sudo mkdir -p $PREFIX/share/claident/bin || exit $?
