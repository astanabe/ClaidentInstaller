...
wget -nv -c https://mafft.cbrc.jp/alignment/software/mafft-7.490-linux.tgz || exit $?
tar -xzf mafft-7.490-linux.tgz || exit $?
mkdir -p $PREFIX/share/claident/bin 2> /dev/null || sudo mkdir -p $PREFIX/share/claident/bin || exit $?
...
rm -rf mafft-7.490-linux.tgz mafft-linux64 mafft-linux32 || exit $?
