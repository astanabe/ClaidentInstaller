...
gnutar -xzf Claident-0.9.2021.07.28.tar.gz || exit $?
...
gnutar -xzf swarm-3.1.0.tar.gz || exit $?
...
gnutar -xzf vsearch-2.17.1.tar.gz || exit $?
...
gnutar -xzf vsearch5d-2.17.1.tar.gz || exit $?
...
gnutar -xzf ncbi-blast-2.12.0+-x64-macosx.tar.gz || exit $?
...
gnutar -xzf R-4.1.0.tar.gz || exit $?
cd R-4.1.0 || exit $?
