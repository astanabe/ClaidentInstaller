...
wget -nv -c https://github.com/astanabe/Claident/archive/v0.9.2022.04.17.tar.gz -O Claident-0.9.2022.04.17.tar.gz || exit $?
gnutar -xzf Claident-0.9.2022.04.17.tar.gz || exit $?
cd Claident-0.9.2022.04.17 || exit $?
...
cd .. || exit $?
rm -rf Claident-0.9.2022.04.17 || exit $?
rm -f Claident-0.9.2022.04.17.tar.gz || exit $?
touch .claident || exit $?
fi
...
gnutar -xzf vsearch-2.21.1.tar.gz || exit $?
...
gnutar -xzf vsearch5d-2.21.1.tar.gz || exit $?
...
gnutar -xzf ncbi-blast-2.13.0+-x64-macosx.tar.gz || exit $?
...
gnutar -xzf R-4.1.3.tar.gz || exit $?
cd R-4.1.3 || exit $?
